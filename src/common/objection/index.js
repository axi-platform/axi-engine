 ...
 * @param {string} [eager] - Objection eager loading string.
 * @param {string} [allowedEager] - Objection eager loading string.
 */

export default class ObjectionService {
  constructor(options) {
    if (!options) {
  ...
  createQuery(paramsQuery = {}) {
    ...
    let q = this.Model.query()
      .skipUndefined()
      .eager(this.eager, this.namedEagerFilters)
    ...
    if (query && query.$joinEager) {
      ...
      q.eagerAlgorithm(this.Model.JoinEagerAlgorithm).eager(
        $joinEager,
        this.namedEagerFilters,
      )
    }
